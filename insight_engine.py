...
try:
    ...
    전문가로서 단순히 뉴스를 전달하는 것이 아니라, 시장의 심리와 기술적 위치를 분석하여 리포트를 작성하세요.

    [시장 상태 데이터]
    - 거래 가능 여부: 한국({kr_trading_status}), 미국({us_trading_status})
    ...
    1. **종목 구성 비율 강제 규칙**:
       ...
       - 추천종목 tickers 배열에는 반드시 종목 코드(숫자)가 아닌 사람이 읽을 수 있는 종목명(예: '삼성전자', 'SK하이닉스', 'NVIDIA')으로 작성하세요.
    ...
    3. [출력 양식]: 반드시 아래 JSON 형식으로만 답변하고 앞뒤 설명은 생략하세요.
